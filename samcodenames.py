import random
import os
from Samtry import get_clue, split_board
...
class Board:
    ...
    def __init__(self, cols, rows):
        ...
        self.cols = cols
        self.rows = rows
        self.word_list_file = "/home/zackss/comps2024/word_list.txt"
...
class Game():
    ...
    def start_game(self):
        ...
        while self.playing:
            ...
            if self.turn == "Red":
                if ai_status_red:
                    words_obj = split_board(self.board, self.turn.lower())
                    clue_obj = get_clue(words_obj, red_given_clues, self.board)
                    clue, number = clue_obj
                ...
                if ai_status_blue:
                    words_obj = split_board(self.board, self.turn.lower())
                    clue_obj = get_clue(words_obj, blue_given_clues, self.board)
                    clue, number = clue_obj
                    number = len(clue_obj[0])
                    prompted_words = clue_obj[0]
                else: 
            ...
            while not turn_complete:
                ...
                print(f"The clue your teammate gave is \"{clue}\" for 2")
